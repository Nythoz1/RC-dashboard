...
const EMPTY={
  ...
  boms:BOM_SEED,bomSheets:[],vendors:VENDOR_SEED,
...
const syncLabor=st=>{
  const jobEng={};(st.jobs||[]).forEach(j=>{if(j.engineId)jobEng[j.id]=+j.engineId;});
...
const LABOR_LISTS=["jobs","timeEntries","diagnoses"];
...
function reducer(s,a){switch(a.type){
  case "TAB":return{...s,tab:a.v,mstack:[]};case "MODAL":{const st=s.mstack||[];if(s.modal===a.v)return{...s,md:a.d||null};const ix=st.findIndex(x=>x.modal===a.v);if(ix>=0)return{...s,modal:a.v,md:a.d||null,mstack:st.slice(0,ix)};if(!s.modal)return{...s,modal:a.v,md:a.d||null,mstack:[]};return{...s,modal:a.v,md:a.d||null,mstack:[...st,{modal:s.modal,md:s.md}].slice(-8)};}
...
const invTot=inv=>((inv.items||[]).reduce((s,i)=>s+(i.q||0)*(i.r||0),0))*1.05;
const qTot=q=>(q.items||[]).reduce((s,i)=>s+(i.q||0)*(i.r||0),0);
const qTax=q=>qTot(q)*1.05;
const stk=i=>i.qty===0?"out":i.qty<=i.reorder?"low":"ok";
const cn=(c,id)=>((c||[]).find(x=>x.id===id)||{}).name||"Unknown";
...
// breakdown was entered) + itemized parts + diagnosis parts + auto labor.
// The flat cost is the acquisition cost, so logging a $50 part never drops the basis to $50.
const fixedCost=i=>{const b=(+i.costCore||0)+(+i.costFreight||0)+(+i.costParts||0)+(+i.costLabor||0);return b>0?b:(+i.cost||0);};
...
const engLinks=(s,id)=>({
  invoice:(s.invoices||[]).find(x=>x.engineId===id),
  core:(s.cores||[]).find(x=>x.engineId===id),
  warranty:(s.warranties||[]).find(x=>x.engineId===id),
  shipment:(s.shipments||[]).find(x=>x.engineId===id),
});
...
function printEngine(s,i){
  ...
  const jl=(s.jobs||[]).filter(j=>j.engineId===i.id).map(j=>{const te=(s.timeEntries||[]).filter(t=>t.jobId===j.id);return{j,h:te.reduce((a,t)=>a+(+t.hours||0),0),v:te.reduce((a,t)=>a+(+t.hours||0)*(+t.rate||0),0)};}).filter(x=>x.h>0||x.v>0);
  ...
  '<style>@import url("https://fonts.googleapis.com/css2?family=Barlow+Condensed:wght@600;700;800&family=IBM+Plex+Mono:wght@400;500;600&display=swap");'+
...
const BC={ok:"var(--g)","in-stock":"var(--g)",paid:"var(--g)",complete:"var(--g)",active:"var(--g)",confirmed:"var(--g)",converted:"var(--g)",approved:"var(--g)",received:"var(--g)",accepted:"var(--g)",credited:"var(--g)",delivered:"var(--g)",low:"var(--w)",pending:"var(--w)",queued:"var(--mt)","on-leave":"var(--w)",contacted:"var(--b)",quoted:"var(--b)",negotiating:"var(--p)","in-progress":"var(--b)",scheduled:"var(--b)",sent:"var(--b)",shipped:"var(--b)",inspected:"var(--b)",ordered:"var(--b)",out:"var(--r)",overdue:"var(--r)",high:"var(--r)",lost:"var(--r)",declined:"var(--r)",rejected:"var(--r)",expired:"var(--r)",claimed:"var(--w)",medium:"var(--w)",new:"var(--ac)",draft:"var(--mt)","in-transit":"var(--b)",available:"var(--g)","in-reman":"var(--p)","on-hold":"var(--w)",sold:"var(--r)",open:"var(--ac)",monitoring:"var(--w)",resolved:"var(--g)",seed:"var(--mt)",shop:"var(--ac)"};
function Badge({s}){const c=BC[s]||"var(--mt)";return (<span style={{display:"inline-flex",alignItems:"center",gap:5,padding:"3px 10px",borderRadius:20,fontSize:11,fontWeight:700,letterSpacing:1,textTransform:"uppercase",fontFamily:"var(--fb)",color:c,border:"1px solid "+tint(c,40),background:tint(c,12)}}><span style={{width:6,height:6,borderRadius:"50%",background:c}}/>{(s||"").replace(/-/g," ")}</span>);}
...
function Tbl({headers,children}){return (<div className="rc-card"><table className="rc-tbl"><thead><tr>{headers.map((h,i)=>(<th key={i}>{h}</th>))}</tr></thead><tbody>{children}</tbody></table></div>);}
function BtnRow({children}){return (<div style={{display:"flex",gap:4,flexWrap:"wrap"}}>{children}</div>);}
...
function Overview({s,d}){
  ...
  const overdueInv=s.invoices.filter(i=>i.status==="overdue").length;
  ...
  const cmKey=new Date().toISOString().slice(0,7);
  const mWins=(s.wins||[]).filter(w=>w.kind==="sale"&&(w.ts||"").slice(0,7)===cmKey);
  const mRev=mWins.reduce((a,w)=>a+(+w.price||0),0);
  const prevBest=Object.values((s.wins||[]).filter(w=>w.kind==="sale"&&(w.ts||"").slice(0,7)!==cmKey).reduce((m,w)=>{const k=(w.ts||"").slice(0,7);m[k]=(m[k]||0)+(+w.price||0);return m;},{})).reduce((a,v)=>Math.max(a,v),0);
  ...
  const avgMarg=costWins.length?costWins.reduce((a,w)=>a+((+w.price||0)-(+w.cost||0)),0)/costWins.length:((s.wins||[]).length?(s.wins||[]).reduce((a,w)=>a+(+w.price||0),0)/(s.wins||[]).length*.3:0);
  const beEng=fixedMo>0&&avgMarg>0?Math.ceil(fixedMo/avgMarg):null;
  const bePct=fixedMo>0&&goal>0?Math.min(100,fixedMo/goal*100):null;
  return (<div>
    <div className="rc-card rc-goal">
      ...
      <div className="rc-goal-bar"><div className="rc-goal-fill" style={{width:pct+"%",background:pct>=100?"linear-gradient(90deg,var(--g),var(--g))":"var(--grad)"}}/>{bePct!=null&&bePct<100&&<div className="rc-goal-mark" style={{left:bePct+"%"}} title={"break-even "+$K(fixedMo)}/>}</div>
      <div className="rc-goal-sub">{mWins.length} engine{mWins.length===1?"":"s"} sold this month · {pct.toFixed(0)}% of goal{fixedMo>0?(<span> · break-even {$K(fixedMo)}{beEng?` ≈ ${beEng} engine${beEng>1?"s":""}`:""}{mRev>=fixedMo?" ✓ cleared":""}</span>):(<span style={{color:"var(--ft)"}}> · add monthly expenses to see your break-even line</span>)}</div>
    </div>
    {(()=>{const b=justSent&&justSent.text?{date:isoToday(),text:justSent.text,sent:justSent.sent,to:justSent.to,reason:justSent.reason}:(s.brief&&s.brief.date?s.brief:null);return(<div className="rc-card" style={{padding:"12px 16px",marginBottom:16}}>
      ...
      {b&&showBrief&&<pre style={{whiteSpace:"pre-wrap",fontSize:13,lineHeight:1.65,color:"var(--tx)",marginTop:10,fontFamily:"var(--fb)",background:"var(--in)",border:"1px solid var(--ln)",borderRadius:9,padding:"10px 12px",maxHeight:360,overflowY:"auto"}}>{b.text}</pre>}
    </div>);})()}
    ...
    <div className="rc-2col">
      ...
      <div><SH title="Wins"/><div className="rc-card" style={{padding:"10px 14px"}}>
        {(s.wins||[]).length===0?(<div style={{fontSize:13,color:"var(--mt)",padding:"8px 0"}}>Your first SOLD lands here. 🏆</div>):(s.wins||[]).slice(0,6).map(w=>(<div key={w.id} className="rc-win"><span className="rc-win-i">🏆</span><span style={{flex:1,minWidth:0}}><span className="rc-tn">{w.name}</span><span className="rc-win-d"> · {(w.ts||"").slice(0,10)} · {w.user}</span></span>{w.src?(<span style={{fontSize:10,color:"var(--b)",border:"1px solid var(--b)",borderRadius:8,padding:"1px 6px",flexShrink:0}}>{srcLabel(w.src)}</span>):(<select value="" onChange={e=>e.target.value&&d({type:"UPDATE",list:"wins",id:w.id,d:{src:e.target.value}})} style={{background:"var(--in)",border:"1px solid var(--ln)",borderRadius:6,color:"var(--tx2)",fontSize:10,padding:"1px 2px",flexShrink:0,fontFamily:"var(--fb)"}}><option value="">src?</option>{SALE_SRC.map(([k,l])=>(<option key={k} value={k}>{l}</option>))}</select>)}<span className="rc-win-p">{$$(w.price)}</span></div>))}
      </div></div>
    </div>
  </div>);
}
...
function Customers({s,d}){
  ...
  const jFilt=(s.jobs||[]).filter(j=>jf==="all"||j.status===jf);
  if(view==="board"){const cols=[{k:"queued",l:"Queued"},{k:"in-progress",l:"In Progress"},{k:"complete",l:"Complete"}];return (<div><SH title="Job Board"><button className="rc-fb" onClick={()=>setView("list")}>← Back</button><button className="rc-ba" onClick={()=>d({type:"MODAL",v:"add-job"})}>+ Job</button></SH><div className="rc-board">{cols.map(col=>{const cj=s.jobs.filter(j=>j.status===col.k);return (<div key={col.k} className="rc-bcol"><div className="rc-bh"><span className="rc-bt">{col.l}</span><span className="rc-bc">{cj.length}</span></div><div style={{padding:10,display:"flex",flexDirection:"column",gap:8}}>{cj.map(j=>(<div key={j.id} className={"rc-jc "+(j.priority||"")} onClick={()=>d({type:"MODAL",v:"job-detail",d:j})}><div className="rc-jcn">{jobCust(s,j)}</div><div style={{fontSize:13,color:"var(--ac)"}}>{j.service}</div><div style={{fontSize:12,color:"var(--mt)",display:"flex",justifyContent:"space-between",marginTop:4}}><span>{j.tech}</span><span>{j.due}</span></div></div>))}</div></div>);})}</div></div>);}
  return (<div>
    ...
    {filt.length===0?(<Empty icon="👤" title="No Customers" sub="Add your first customer" action={()=>d({type:"MODAL",v:"add-cust"})} label="+ Add"/>):(<div className="rc-gc">{filt.map(c=>(<div key={c.id} className="rc-cc" onClick={()=>d({type:"MODAL",v:"cust-detail",d:c})}><div style={{display:"flex",justifyContent:"space-between"}}><div><div className="rc-ccn">{c.name}</div><div style={{fontSize:13,color:"var(--ac)"}}>{c.type||"Individual"} {c.province?`· ${c.province}`:""}</div></div><BtnRow><button className="rc-bs" onClick={e=>{e.stopPropagation();d({type:"MODAL",v:"edit-cust",d:c})}} style={{fontSize:14.5}}>✎</button><button className="rc-bs rc-bsr" onClick={e=>{e.stopPropagation();d({type:"DELETE",list:"customers",id:c.id})}} style={{fontSize:14.5}}>×</button></BtnRow></div><div style={{fontSize:13,color:"var(--tx2)",marginTop:4}}>{c.phone} · {c.email}</div><div className="rc-3c" style={{margin:"10px 0"}}><div><div className="rc-ml">Spent</div><div className="rc-mv">{$K(c.spent||0)}</div></div><div><div className="rc-ml">Visits</div><div className="rc-mv">{c.visits||0}</div></div><div><div className="rc-ml">Last</div><div className="rc-mv" style={{fontSize:14.5}}>{c.last||"—"}</div></div></div>{(c.vehicles||[]).length>0&&<div style={{fontSize:13,color:"var(--tx2)"}}>{c.vehicles.join(" · ")}</div>}</div>))}</div>)}
    <SH title="Work Orders"><button className="rc-ba" onClick={()=>d({type:"MODAL",v:"add-job"})}>+ Job</button></SH>
    <Fil opts={[["all","All"],["queued","Queued"],["in-progress","Active"],["complete","Done"]]} active={jf} set={setJf}/>
    {jFilt.length===0?(<Empty icon="🔧" title="No Jobs" sub="Create work orders" action={()=>d({type:"MODAL",v:"add-job"})} label="+ Job"/>):(<Tbl headers={["Customer","Vehicle","Service","Tech","Due","Status",""]}>{jFilt.map(j=>(<tr key={j.id}><td className="rc-tn">{jobCust(s,j)}</td><td style={{fontSize:13,color:"var(--tx2)"}}>{j.vehicle}</td><td style={{fontSize:13,color:"var(--ac)"}}>{j.service}</td><td style={{fontSize:13}}>{j.tech}</td><td style={{fontSize:13,color:"var(--mt)"}}>{j.due}</td><td><Badge s={j.status}/></td><td><BtnRow><button className="rc-bs" onClick={()=>d({type:"MODAL",v:"edit-job",d:j})} style={{fontSize:14.5}}>✎</button>{j.status!=="complete"&&<button className="rc-bs" onClick={()=>d({type:"UPDATE",list:"jobs",id:j.id,d:{status:j.status==="queued"?"in-progress":"complete"}})}>→</button>}<button className="rc-bs rc-bsr" onClick={()=>d({type:"DELETE",list:"jobs",id:j.id})} style={{fontSize:14.5}}>×</button></BtnRow></td></tr>))}</Tbl>)}
    <SH title="Communication Log"><button className="rc-ba" onClick={()=>d({type:"MODAL",v:"add-comm"})}>+ Log</button></SH>
    {(s.commsLog||[]).length===0?(<Empty icon="💬" title="No Logs" sub="Track calls, emails, texts" action={()=>d({type:"MODAL",v:"add-comm"})} label="+ Add"/>):(<Tbl headers={["Date","Customer","Type","Summary","Follow-Up",""]}>{(s.commsLog||[]).slice(0,10).map(c=>(<tr key={c.id}><td style={{fontSize:13}}>{c.date}</td><td className="rc-tn">{cn(s.customers,c.custId)}</td><td><Badge s={c.type}/></td><td style={{fontSize:13,color:"var(--tx2)"}}>{c.summary}</td><td style={{fontSize:13,color:c.followUp?"var(--w)":"var(--mt)"}}>{c.followUp||"—"}</td><td><BtnRow><button className="rc-bs" onClick={()=>d({type:"MODAL",v:"edit-comm",d:c})} style={{fontSize:14.5}}>✎</button><button className="rc-bs rc-bsr" onClick={()=>d({type:"DELETE",list:"commsLog",id:c.id})} style={{fontSize:14.5}}>×</button></BtnRow></td></tr>))}</Tbl>)}
  </div>);
}
...
function Quotes({s,d}){
  ...
  return (<div>
    ...
    <Fil opts={[["all","All"],["draft","Draft"],["sent","Sent"],["approved","Approved"],["declined","Declined"]]} active={f} set={sf}/>
    {aiR&&(<div className="rc-card" style={{padding:16,marginBottom:16,borderColor:"var(--p)"}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:10}}><span style={{fontFamily:"var(--fd)",fontWeight:700,color:"var(--p)",letterSpacing:2,textTransform:"uppercase",fontSize:14.5}}>✨ AI Quote Draft</span><button className="rc-bs" onClick={()=>setAiR(null)} style={{fontSize:14.5}}>✕</button></div><div style={{fontSize:14,color:"var(--tx2)",marginBottom:8}}>{aiR.description}</div><div style={{background:"var(--sf2)",borderRadius:6,padding:10,marginBottom:10}}>{(aiR.items||[]).map((it,i)=>(<div key={i} style={{display:"flex",justifyContent:"space-between",padding:"5px 0",borderBottom:i<aiR.items.length-1?"1px solid var(--ln)":"none",fontSize:14}}><span style={{flex:1}}>{it.d}</span><span style={{width:80,textAlign:"right",fontWeight:600}}>{$$(it.q*it.r)}</span></div>))}</div><BtnRow><button className="rc-ba" onClick={()=>{d({type:"ADD",list:"quotes",d:{quoteNum:"QT-"+Date.now().toString().slice(-6),custId:0,description:aiR.description,items:aiR.items,notes:aiR.notes||"",status:"draft",date:today(),validUntil:"30 days"}});setAiR(null);}}>Save Draft</button><button className="rc-bs" onClick={aiGen} style={{color:"var(--p)",borderColor:"var(--p)"}}>🔄</button></BtnRow></div>)}
    {fl.length===0?(<Empty icon="📋" title="No Quotes" sub="Create estimates" action={()=>d({type:"MODAL",v:"add-quote"})} label="+ Quote"/>):(<Tbl headers={["#","Customer","Description","Total","Status","Actions"]}>{fl.map(q=>(<tr key={q.id}><td style={{fontFamily:"var(--fd)",fontWeight:700,color:"var(--ac)"}}>{q.quoteNum||q.id}</td><td className="rc-tn">{cn(s.customers,q.custId)}</td><td style={{fontSize:13,color:"var(--tx2)"}}>{q.description}</td><td style={{fontWeight:600}}>{$$(qTax(q))}</td><td><Badge s={q.status}/></td><td><BtnRow><button className="rc-bs" onClick={()=>d({type:"MODAL",v:"quote-detail",d:q})}>View</button><button className="rc-bs" onClick={()=>d({type:"MODAL",v:"edit-quote",d:q})} style={{fontSize:14.5}}>✎</button>{q.status==="draft"&&<button className="rc-bs" style={{color:"var(--b)",borderColor:"var(--b)"}} onClick={()=>d({type:"UPDATE",list:"quotes",id:q.id,d:{status:"sent"}})}>Send</button>}{q.status==="sent"&&<><button className="rc-bs rc-bsg" onClick={()=>d({type:"UPDATE",list:"quotes",id:q.id,d:{status:"approved"}})}>✓</button><button className="rc-bs rc-bsr" onClick={()=>d({type:"UPDATE",list:"quotes",id:q.id,d:{status:"declined"}})}>✗</button></>}{q.status==="approved"&&<><button className="rc-bs" style={{color:"var(--g)",borderColor:"var(--g)"}} onClick={()=>d({type:"ADD",list:"jobs",d:{custId:q.custId,vehicle:"",service:q.description,type:"Engine Sale",tech:"Unassigned",due:"",notes:"From "+q.quoteNum,priority:"medium",status:"queued"}})}>→Job</button><button className="rc-bs" style={{color:"var(--ac)",borderColor:"var(--ac)"}} onClick={()=>d({type:"ADD",list:"invoices",d:{invNum:"INV-"+Date.now().toString().slice(-6),custId:q.custId,date:today(),due:"Net 30",items:q.items,status:"pending"}})}>→Inv</button></>}<button className="rc-bs rc-bsr" onClick={()=>d({type:"DELETE",list:"quotes",id:q.id})} style={{fontSize:14.5}}>×</button></BtnRow></td></tr>))}</Tbl>)}
  </div>);
}
...
function Inv({s,d}){
  ...
  const matchQ=i=>{const t=(q||"").toLowerCase();return !t||[i.name,i.sku,i.serial,i.esn,i.cpl].some(v=>(v||"").toLowerCase().includes(t));};
...
function Parts({s,d}){
  ...
  return (<div>
    ...
    {fl.length===0?(<Empty icon="🔧" title="No Injectors" sub="Add injector cross-reference parts" action={()=>d({type:"MODAL",v:"add-injector"})} label="+ Add Injector"/>):(<Tbl headers={["Brand","Engine","Family","OEM Part #","Aftermarket #","Type","Cond","Qty","Cost","Sell","Margin",""]}>{fl.map(p=>{const m=p.sell>0?((p.sell-(p.cost||0))/p.sell*100):0;return (<tr key={p.id}><td className="rc-tn">{p.brand}</td><td style={{fontSize:13,color:"var(--ac)"}}>{p.engine}</td><td style={{fontSize:13,color:"var(--tx2)"}}>{p.family||"—"}</td><td style={{fontSize:13}}>{p.oem||"—"}</td><td style={{fontSize:13,color:"var(--tx2)"}}>{p.aftermarket||"—"}</td><td style={{fontSize:13}}>{p.type||"—"}</td><td style={{fontSize:13}}>{p.cond||"—"}</td><td style={{fontWeight:600}}>{p.qty||0}</td><td style={{fontSize:13,color:"var(--tx2)"}}>{p.cost>0?$$(p.cost):"—"}</td><td style={{color:"var(--ac)",fontWeight:600}}>{p.sell>0?$$(p.sell):"—"}</td><td style={{fontSize:13,color:m>40?"var(--g)":"var(--w)"}}>{p.sell>0&&p.cost>0?m.toFixed(0)+"%":"—"}</td><td><BtnRow><button className="rc-bs" onClick={()=>d({type:"MODAL",v:"edit-injector",d:p})} style={{fontSize:14.5}}>✎</button><button className="rc-bs rc-bsr" onClick={()=>d({type:"DELETE",list:"parts",id:p.id})} style={{fontSize:14.5}}>×</button></BtnRow></td></tr>);})}</Tbl>)}
  </div>);
}
...
function Issues({s,d}){
  ...
  return (<div>
    ...
    {view==="issues"?(<>
      ...
      {fl.length===0?(<Empty icon="📚" title="No Issues" sub="Nothing matches — add what you've learned on the floor" action={()=>d({type:"MODAL",v:"add-issue"})} label="+ Add Issue"/>):(<Tbl headers={["Sev","Issue","Models","Symptoms","On lot","Seen",""]}>{fl.map(is=>(<tr key={is.id}><td><span style={{display:"inline-block",width:8,height:8,borderRadius:"50%",background:sevCol(is.severity),boxShadow:"0 0 6px "+sevCol(is.severity)}} title={is.severity}/></td><td onClick={()=>d({type:"MODAL",v:"issue-detail",d:is})} style={{cursor:"pointer",maxWidth:340}}><div className="rc-tn">{is.title}</div>{is.causes&&<div style={{fontSize:12,color:"var(--mt)",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",maxWidth:340}}>{is.causes}</div>}</td><td style={{fontSize:13,color:"var(--ac)"}}>{(is.models||[]).length?is.models.map(famLabel).join(", "):<span style={{color:"var(--mt)"}}>all</span>}</td><td><div style={{display:"flex",gap:3,flexWrap:"wrap",maxWidth:220}}>{(is.symptoms||[]).slice(0,4).map(x=>(<span key={x} style={{fontSize:10,fontFamily:"var(--fb)",color:"var(--tx2)",border:"1px solid var(--ln)",borderRadius:2,padding:"0 4px"}}>{x}</span>))}{(is.symptoms||[]).length>4&&<span style={{fontSize:10,color:"var(--mt)"}}>+{is.symptoms.length-4}</span>}</div></td><td style={{fontSize:13,color:onLot(is)>0?"var(--w)":"var(--mt)"}}>{onLot(is)||"—"}</td><td style={{fontSize:13,color:seen(is)>0?"var(--g)":"var(--mt)"}}>{seen(is)||"—"}</td><td><BtnRow><button className="rc-bs" onClick={()=>d({type:"MODAL",v:"edit-issue",d:is})} style={{fontSize:14.5}}>✎</button><button className="rc-bs rc-bsr" onClick={()=>d({type:"DELETE",list:"issues",id:is.id})} style={{fontSize:14.5}}>×</button></BtnRow></td></tr>))}</Tbl>)}
    </>):(<>
      <Fil opts={[["all","All"],...DX_OUTCOMES]} active={fo} set={sfo}/>
      {dxl.length===0?(<Empty icon="🩺" title="No Diagnoses" sub="Log what you find on an engine — from its passport or right here" action={()=>d({type:"MODAL",v:"add-dx",d:{prefill:{date:isoToday()}}})} label="+ Log Diagnosis"/>):(<Tbl headers={["Date","Engine","Symptoms","Findings → Fix","Tech","Cost","Outcome",""]}>{dxl.map(x=>{const e=engById(s,x.engineId);const c=(+x.hours||0)*(+x.rate||0)+(x.parts||[]).reduce((a,p)=>a+(+p.v||0),0);return(<tr key={x.id}><td style={{fontSize:13,color:"var(--tx2)",whiteSpace:"nowrap"}}>{x.date}</td><td onClick={()=>e&&d({type:"MODAL",v:"part-detail",d:{...e,ptab:"diagnosis"}})} style={{cursor:e?"pointer":"default"}}><div className="rc-tn">{e?e.name:"—"}</div>{e&&<div style={{fontSize:11,color:"var(--mt)"}}>{e.sku}</div>}</td><td style={{fontSize:13}}>{(x.symptoms||[]).join(", ")||"—"}</td><td onClick={()=>d({type:"MODAL",v:"dx-detail",d:x})} style={{cursor:"pointer",fontSize:13,maxWidth:300}}><div style={{whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",maxWidth:300}}>{x.findings||"—"}</div>{x.fix&&<div style={{color:"var(--g)",fontSize:12,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",maxWidth:300}}>→ {x.fix}</div>}</td><td style={{fontSize:13,color:"var(--ac)"}}>{x.tech||"—"}</td><td style={{fontSize:13,color:"var(--tx2)"}}>{c>0?$$(c):"—"}</td><td><Badge s={x.outcome||"open"}/></td><td><BtnRow><button className="rc-bs" onClick={()=>d({type:"MODAL",v:"edit-dx",d:x})} style={{fontSize:14.5}}>✎</button><button className="rc-bs rc-bsr" onClick={()=>d({type:"DELETE",list:"diagnoses",id:x.id})} style={{fontSize:14.5}}>×</button></BtnRow></td></tr>);})}</Tbl>)}
    </>)}
  </div>);
}
...
function Invoicing({s,d}){
  const[f,sf]=useState("all");const fl=(s.invoices||[]).filter(inv=>f==="all"||inv.status===f);
  const by=st=>(s.invoices||[]).filter(i=>i.status===st).reduce((a,inv)=>a+invTot(inv),0);
  // AR Aging buckets (simplified — based on status since we don't have exact dates as Date objects)
  const pendingInvs=s.invoices.filter(i=>i.status==="pending");const overdueInvs=s.invoices.filter(i=>i.status==="overdue");
  return (<div>
    <div className="rc-g4"><Stat label="Paid" value={<span style={{color:"var(--g)"}}>{$K(by("paid"))}</span>}/><Stat label="Pending" value={<span style={{color:"var(--w)"}}>{$K(by("pending"))}</span>}/><Stat label="Overdue" value={<span style={{color:"var(--r)"}}>{$K(by("overdue"))}</span>}/><Stat label="Total Outstanding" value={$K(by("pending")+by("overdue"))}/></div>
    <SH title="Accounts Receivable"/>
    <div className="rc-card" style={{padding:16,marginBottom:16}}>
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:16}}>
        <div><div className="rc-ml">Current (0-30 days)</div><div style={{fontFamily:"var(--fd)",fontWeight:700,fontSize:21,color:"var(--g)",marginTop:4}}>{$$(pendingInvs.reduce((a,i)=>a+invTot(i),0))}</div><div style={{fontSize:12,color:"var(--mt)"}}>{pendingInvs.length} invoice{pendingInvs.length!==1?"s":""}</div></div>
        <div><div className="rc-ml">Overdue (31-60 days)</div><div style={{fontFamily:"var(--fd)",fontWeight:700,fontSize:21,color:"var(--w)",marginTop:4}}>{$$(overdueInvs.filter((_,i)=>i<Math.ceil(overdueInvs.length/2)).reduce((a,i)=>a+invTot(i),0))}</div></div>
        <div><div className="rc-ml">Severely Overdue (60+)</div><div style={{fontFamily:"var(--fd)",fontWeight:700,fontSize:21,color:"var(--r)",marginTop:4}}>{$$(overdueInvs.filter((_,i)=>i>=Math.ceil(overdueInvs.length/2)).reduce((a,i)=>a+invTot(i),0))}</div></div>
      </div>
    </div>
    <SH title="Invoices"><button className="rc-ba" onClick={()=>d({type:"MODAL",v:"add-inv"})}>+ Invoice</button></SH>
    <Fil opts={[["all","All"],["pending","Pending"],["paid","Paid"],["overdue","Overdue"]]} active={f} set={sf}/>
    {fl.length===0?(<Empty icon="📄" title="No Invoices" sub="Create invoices" action={()=>d({type:"MODAL",v:"add-inv"})} label="+ Invoice"/>):(<Tbl headers={["#","Customer","Date","Terms","Total","Status","Actions"]}>{fl.map(inv=>(<tr key={inv.id}><td style={{fontFamily:"var(--fd)",fontWeight:700,color:"var(--ac)"}}>{inv.invNum||inv.id}</td><td className="rc-tn">{cn(s.customers,inv.custId)}</td><td style={{fontSize:13,color:"var(--mt)"}}>{inv.date}</td><td style={{fontSize:13,color:inv.status==="overdue"?"var(--r)":"var(--mt)"}}>{inv.due}</td><td style={{fontWeight:600}}>{$$(invTot(inv))}</td><td><Badge s={inv.status}/></td><td><BtnRow><button className="rc-bs" onClick={()=>d({type:"MODAL",v:"inv-detail",d:inv})}>View</button><button className="rc-bs" onClick={()=>d({type:"MODAL",v:"edit-inv",d:inv})} style={{fontSize:14.5}}>✎</button>{inv.status==="pending"&&<button className="rc-bs rc-bsg" onClick={()=>d({type:"UPDATE",list:"invoices",id:inv.id,d:{status:"paid"}})}>Paid</button>}{inv.status==="overdue"&&<button className="rc-bs rc-bsr" onClick={()=>d({type:"UPDATE",list:"invoices",id:inv.id,d:{status:"paid"}})}>Resolve</button>}<button className="rc-bs rc-bsr" onClick={()=>d({type:"DELETE",list:"invoices",id:inv.id})} style={{fontSize:14.5}}>×</button></BtnRow></td></tr>))}</Tbl>)}
  </div>);
}
...
function Operations({s,d}){
  ...
  return (<div>
    ...
    {tab==="cores"&&(<div>
      ...
      {(s.cores||[]).length===0?(<Empty icon="🔄" title="No Core Tracking" sub="Track engine core returns" action={()=>d({type:"MODAL",v:"add-core"})} label="+ Add Core"/>):(<Tbl headers={["Engine","Customer","Deposit","Due Date","Status",""]}>{(s.cores||[]).map(c=>(<tr key={c.id}><td className="rc-tn">{c.engineName}</td><td style={{fontSize:13}}>{cn(s.customers,c.custId)}</td><td style={{fontWeight:600,color:"var(--ac)"}}>{$$(c.deposit||0)}</td><td style={{fontSize:13,color:c.status==="pending"?"var(--w)":"var(--mt)"}}>{c.dueDate||"—"}</td><td><Badge s={c.status}/></td><td><BtnRow><button className="rc-bs" onClick={()=>d({type:"MODAL",v:"edit-core",d:c})} style={{fontSize:14.5}}>✎</button><select className="rc-fi" value={c.status} onChange={e=>d({type:"UPDATE",list:"cores",id:c.id,d:{status:e.target.value}})} style={{width:100,padding:"3px 6px",fontSize:12}}>{["pending","received","inspected","accepted","rejected","credited"].map(st=>(<option key={st} value={st}>{st}</option>))}</select><button className="rc-bs rc-bsr" onClick={()=>d({type:"DELETE",list:"cores",id:c.id})} style={{fontSize:14.5}}>×</button></BtnRow></td></tr>))}</Tbl>)}
    </div>)}

    {tab==="ship"&&(<div>
      <SH title="Shipments"><button className="rc-ba" onClick={()=>d({type:"MODAL",v:"add-ship"})}>+ Shipment</button></SH>
      {(s.shipments||[]).length===0?(<Empty icon="🚚" title="No Shipments" sub="Track engine shipments" action={()=>d({type:"MODAL",v:"add-ship"})} label="+ Add"/>):(<Tbl headers={["Customer","Carrier","Tracking","Freight","Ship Date","Status",""]}>{(s.shipments||[]).map(sh=>(<tr key={sh.id}><td className="rc-tn">{cn(s.customers,sh.custId)}</td><td style={{fontSize:13}}>{sh.carrier}</td><td style={{fontSize:13,color:"var(--b)"}}>{sh.tracking||"—"}</td><td style={{fontWeight:600}}>{$$(sh.freightCost||0)}</td><td style={{fontSize:13,color:"var(--mt)"}}>{sh.shipDate}</td><td><Badge s={sh.deliveryConfirmed?"delivered":"in-transit"}/></td><td><BtnRow>{!sh.deliveryConfirmed&&<button className="rc-bs rc-bsg" onClick={()=>d({type:"UPDATE",list:"shipments",id:sh.id,d:{deliveryConfirmed:true}})}>✓ Delivered</button>}<button className="rc-bs" onClick={()=>d({type:"MODAL",v:"edit-ship",d:sh})} style={{fontSize:14.5}}>✎</button><button className="rc-bs rc-bsr" onClick={()=>d({type:"DELETE",list:"shipments",id:sh.id})} style={{fontSize:14.5}}>×</button></BtnRow></td></tr>))}</Tbl>)}
    </div>)}

    {tab==="po"&&(<div>
      <SH title="Purchase Orders"><button className="rc-ba" onClick={()=>d({type:"MODAL",v:"add-po"})}>+ PO</button></SH>
      {(s.purchaseOrders||[]).length===0?(<Empty icon="📦" title="No POs" sub="Track parts orders" action={()=>d({type:"MODAL",v:"add-po"})} label="+ Add"/>):(<Tbl headers={["Vendor","Order Date","ETA","Total","Status",""]}>{(s.purchaseOrders||[]).map(po=>(<tr key={po.id}><td className="rc-tn">{po.vendor}</td><td style={{fontSize:13}}>{po.orderDate}</td><td style={{fontSize:13,color:"var(--mt)"}}>{po.eta||"—"}</td><td style={{fontWeight:600}}>{$$((po.items||[]).reduce((a,i)=>a+(i.q||0)*(i.r||0),0))}</td><td><Badge s={po.status}/></td><td><BtnRow><button className="rc-bs" onClick={()=>d({type:"MODAL",v:"edit-po",d:po})} style={{fontSize:14.5}}>✎</button><select className="rc-fi" value={po.status} onChange={e=>d({type:"UPDATE",list:"purchaseOrders",id:po.id,d:{status:e.target.value}})} style={{width:90,padding:"3px 6px",fontSize:12}}>{["ordered","shipped","received"].map(st=>(<option key={st} value={st}>{st}</option>))}</select><button className="rc-bs rc-bsr" onClick={()=>d({type:"DELETE",list:"purchaseOrders",id:po.id})} style={{fontSize:14.5}}>×</button></BtnRow></td></tr>))}</Tbl>)}
    </div>)}

    {tab==="warranty"&&(<div>
      <SH title="Warranties"><button className="rc-ba" onClick={()=>d({type:"MODAL",v:"add-warranty"})}>+ Warranty</button></SH>
      {(s.warranties||[]).length===0?(<Empty icon="🛡" title="No Warranties" sub="Track engine warranties" action={()=>d({type:"MODAL",v:"add-warranty"})} label="+ Add"/>):(<Tbl headers={["Engine","Customer","Period","Start","Expiry","Status",""]}>{(s.warranties||[]).map(w=>(<tr key={w.id}><td className="rc-tn">{w.engineName}</td><td style={{fontSize:13}}>{cn(s.customers,w.custId)}</td><td style={{fontSize:13}}>{w.warrantyPeriod}</td><td style={{fontSize:13,color:"var(--mt)"}}>{w.startDate}</td><td style={{fontSize:13,color:w.status==="expired"?"var(--r)":"var(--mt)"}}>{w.expiryDate}</td><td><Badge s={w.status}/></td><td><BtnRow><button className="rc-bs" onClick={()=>d({type:"MODAL",v:"edit-warranty",d:w})} style={{fontSize:14.5}}>✎</button><select className="rc-fi" value={w.status} onChange={e=>d({type:"UPDATE",list:"warranties",id:w.id,d:{status:e.target.value}})} style={{width:90,padding:"3px 6px",fontSize:12}}>{["active","expired","claimed"].map(st=>(<option key={st} value={st}>{st}</option>))}</select><button className="rc-bs rc-bsr" onClick={()=>d({type:"DELETE",list:"warranties",id:w.id})} style={{fontSize:14.5}}>×</button></BtnRow></td></tr>))}</Tbl>)}
    </div>)}
  </div>);
}
...
function Marketing({s,d}){
  ...
  return (<div>
    ...
    <div className="rc-card" style={{padding:16,marginBottom:16,borderColor:"var(--ac)"}}>
      <div style={{fontFamily:"var(--fd)",fontWeight:800,fontSize:17.5,letterSpacing:2,textTransform:"uppercase",color:"var(--ac)"}}>⚡ 1-Post Funnel</div>
      ...
      <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
        ...
        {res&&<button className="rc-ba" onClick={openAll} style={{background:"var(--sf2)",color:"var(--ac)"}}>🚀 Open All 3 + Mark Listed</button>}
      </div>
    ...
    {(s.social||[]).length===0?(<Empty icon="📱" title="No Accounts" sub="Add social media" action={()=>d({type:"MODAL",v:"add-social"})} label="+ Add"/>):(<Tbl headers={["Platform","Handle","Followers","Posts","Engage",""]}>{(s.social||[]).map(ac=>(<tr key={ac.id}><td className="rc-tn">{ac.platform}</td><td style={{fontSize:13,color:"var(--tx2)"}}>{ac.handle}</td><td style={{fontWeight:600}}>{(ac.followers||0).toLocaleString()}</td><td style={{fontSize:13}}>{ac.posts||0}</td><td style={{fontSize:13,color:"var(--ac)"}}>{ac.engagement?ac.engagement+"%":"—"}</td><td><BtnRow><button className="rc-bs" onClick={()=>d({type:"MODAL",v:"edit-social",d:ac})} style={{fontSize:14.5}}>✎</button><button className="rc-bs rc-bsr" onClick={()=>d({type:"DELETE",list:"social",id:ac.id})} style={{fontSize:14.5}}>×</button></BtnRow></td></tr>))}</Tbl>)}
    <SH title="Campaigns"><button className="rc-ba" onClick={()=>d({type:"MODAL",v:"add-campaign"})}>+ Campaign</button></SH>
    {(s.campaigns||[]).length>0&&<Tbl headers={["Name","Platform","Type","Reach","Leads","Spent","Status",""]}>{(s.campaigns||[]).map(c=>(<tr key={c.id}><td className="rc-tn">{c.name}</td><td style={{fontSize:13,color:"var(--tx2)"}}>{c.platform}</td><td style={{fontSize:13}}>{c.type}</td><td style={{fontSize:13}}>{c.reach>0?(c.reach/1000).toFixed(1)+"k":"—"}</td><td style={{fontSize:13}}>{c.leads||"—"}</td><td style={{fontSize:13}}>{c.spent>0?"$"+c.spent:"Free"}</td><td><Badge s={c.status}/></td><td><BtnRow><button className="rc-bs" onClick={()=>d({type:"MODAL",v:"edit-campaign",d:c})} style={{fontSize:14.5}}>✎</button><button className="rc-bs rc-bsr" onClick={()=>d({type:"DELETE",list:"campaigns",id:c.id})} style={{fontSize:14.5}}>×</button></BtnRow></td></tr>))}</Tbl>}
    <SH title="Content Calendar"><button className="rc-ba" onClick={()=>d({type:"MODAL",v:"add-content"})}>+ Entry</button></SH>{(s.contentCalendar||[]).length===0?(<Empty icon="📅" title="No Content Calendar" sub="Plan your weekly posts" action={()=>d({type:"MODAL",v:"add-content"})} label="+ Add"/>):(<Tbl headers={["Day","Content","Platform","Notes",""]}>{(s.contentCalendar||[]).map((c,i)=>(<tr key={c.id||i}><td style={{fontFamily:"var(--fd)",fontWeight:700,color:"var(--ac)"}}>{c.day}</td><td className="rc-tn">{c.type}</td><td style={{fontSize:13,color:"var(--tx2)"}}>{c.platform}</td><td style={{fontSize:13,color:"var(--mt)"}}>{c.notes}</td><td><BtnRow><button className="rc-bs" onClick={()=>d({type:"MODAL",v:"edit-content",d:c})} style={{fontSize:14.5}}>✎</button><button className="rc-bs rc-bsr" onClick={()=>d({type:"DELETE",list:"contentCalendar",id:c.id})} style={{fontSize:14.5}}>×</button></BtnRow></td></tr>))}</Tbl>)}
    ...
    {fL.length===0?(<Empty icon="🎯" title="No Leads" sub="Track leads" action={()=>d({type:"MODAL",v:"add-lead"})} label="+ Lead"/>):(<Tbl headers={["Name","Interest","Source","Prov","Status",""]}>{fL.map(l=>(<tr key={l.id}><td className="rc-tn">{l.name}</td><td style={{color:"var(--ac)",fontSize:13}}>{l.interest}</td><td style={{fontSize:13,color:"var(--tx2)"}}>{l.source}</td><td style={{fontSize:13}}>{l.province}</td><td><Badge s={l.status}/></td><td><BtnRow><button className="rc-bs" onClick={()=>d({type:"MODAL",v:"edit-lead",d:l})} style={{fontSize:14.5}}>✎</button><select className="rc-fi" value={l.status} onChange={e=>d({type:"UPDATE",list:"leads",id:l.id,d:{status:e.target.value}})} style={{width:90,padding:"3px",fontSize:12}}>{["new","contacted","quoted","negotiating","converted","lost"].map(st=>(<option key={st} value={st}>{st}</option>))}</select><button className="rc-bs rc-bsr" onClick={()=>d({type:"DELETE",list:"leads",id:l.id})} style={{fontSize:14.5}}>×</button></BtnRow></td></tr>))}</Tbl>)}
  </div>);
}
...
function Schedule({s,d}){
  const[wo,setWo]=useState(0);const base=new Date();const ws=new Date(base);ws.setDate(ws.getDate()-ws.getDay()+1+wo*7);
...
function Reports({s}){
  ...
  const topCusts=[...(s.customers||[])].sort((a,b)=>(b.spent||0)-(a.spent||0)).slice(0,5);
  ...
  return (<div>
    ...
    <div className="rc-2col">
      {topCats.length>0&&<div><SH title="Inventory by Category"/><div className="rc-card" style={{padding:16}}>{topCats.map(([cat,val],i)=>(<div key={i} style={{marginBottom:12}}><div style={{display:"flex",justifyContent:"space-between",fontSize:14,marginBottom:4}}><span style={{fontFamily:"var(--fd)",fontWeight:600}}>{cat}</span><span style={{color:"var(--ac)",fontWeight:600}}>{$K(val)}</span></div><div style={{height:6,background:"var(--sf2)",borderRadius:3,overflow:"hidden"}}><div style={{height:"100%",width:`${(val/maxCat)*100}%`,background:"var(--ac)",borderRadius:3}}/></div></div>))}</div></div>}
      {topCusts.length>0&&<div><SH title="Top Customers"/><Tbl headers={["#","Customer","Type","Spent"]}>{topCusts.map((c,i)=>(<tr key={c.id}><td style={{fontFamily:"var(--fd)",fontWeight:700,color:i===0?"var(--ac)":"var(--mt)"}}>{i+1}</td><td className="rc-tn">{c.name}</td><td style={{fontSize:13,color:"var(--tx2)"}}>{c.type}</td><td style={{color:"var(--ac)",fontWeight:600}}>{$K(c.spent||0)}</td></tr>))}</Tbl></div>}
    </div>
    {techRows.length>0&&<><SH title="Labor by Technician"/><div className="rc-card" style={{padding:16}}><div style={{display:"flex",justifyContent:"space-between",fontSize:13,color:"var(--mt)",marginBottom:10}}><span>{laborHrs}h logged</span><span style={{color:"var(--ac)",fontWeight:600}}>{$$(laborVal)} billable</span></div>{techRows.map(([tech,v],i)=>(<div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"5px 0",borderBottom:i<techRows.length-1?"1px solid var(--ln)":"none",fontSize:14}}><span style={{fontFamily:"var(--fd)",fontWeight:600}}>{tech}</span><span style={{display:"flex",gap:16,alignItems:"center"}}><span style={{color:"var(--tx2)"}}>{v.hours}h</span><span style={{color:"var(--ac)",fontWeight:600,width:80,textAlign:"right"}}>{$$(v.val)}</span></span></div>))}</div></>}
...
function Modals({s,d}){
  ...
  const F=(k,l,t)=>(<div className="rc-fg"><label className="rc-fl">{l}</label><input className="rc-fi" value={f[k]||""} onChange={e=>set(k,e.target.value)} placeholder={l} type={t||(DATEKEYS.includes(k)?"date":"text")}/></div>);
  ...
  const LI=()=>(<>{lines.map((li,i)=>(<div key={i} style={{display:"grid",gridTemplateColumns:"1fr 50px 70px 28px",gap:6,marginBottom:6}}><input className="rc-fi" placeholder="Description" value={li.d} onChange={e=>{const nl=[...lines];nl[i]={...nl[i],d:e.target.value};setLines(nl);}}/><input className="rc-fi" placeholder="Qty" type="number" value={li.q} onChange={e=>{const nl=[...lines];nl[i]={...nl[i],q:+e.target.value||0};setLines(nl);}}/><input className="rc-fi" placeholder="Rate" type="number" value={li.r} onChange={e=>{const nl=[...lines];nl[i]={...nl[i],r:+e.target.value||0};setLines(nl);}}/><button className="rc-bs" style={{padding:"8px 0"}} onClick={()=>setLines(lines.filter((_,j)=>j!==i))}>×</button></div>))}<button className="rc-bs" onClick={()=>setLines([...lines,{d:"",q:1,r:0}])} style={{marginBottom:10}}>+ Line</button></>);
  const sub=lines.reduce((a,l)=>a+(l.q||0)*(l.r||0),0);

  // Detail modals
  if(s.modal==="job-detail"){const j=s.md;const nx={queued:"in-progress","in-progress":"complete"};const te=(s.timeEntries||[]).filter(t=>t.jobId===j.id);const thrs=te.reduce((a,t)=>a+(+t.hours||0),0);const tlab=te.reduce((a,t)=>a+(+t.hours||0)*(+t.rate||0),0);return W(<div><div className="rc-mt">Work Order</div>{[["Customer",jobCust(s,j)],...(j.engineId?[["Engine",((s.inventory||[]).find(x=>x.id===j.engineId)||{}).name||"—"]]:[]),["Vehicle",j.vehicle],["Service",j.service],["Type",j.type],["Tech",j.tech],["Due",j.due],["Notes",j.notes]].map(([l,v],i)=>(<div key={i} className="rc-fg"><div className="rc-fl">{l}</div><div style={{fontSize:14.5,color:l==="Service"?"var(--ac)":"var(--tx)"}}>{v}</div></div>))}<div className="rc-fg"><div className="rc-fl">Status</div><Badge s={j.status}/></div><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",margin:"12px 0 6px"}}><span className="rc-fl" style={{margin:0}}>Labor — {thrs}h · {$$(tlab)}{j.engineId&&<span style={{color:"var(--g)",marginLeft:6,textTransform:"none"}}>→ engine cost</span>}</span><button className="rc-bs" onClick={()=>d({type:"MODAL",v:"add-time",d:{job:j}})}>+ Log Time</button></div>{te.length>0?<div style={{background:"var(--sf2)",borderRadius:6,padding:8,marginBottom:8}}>{te.map(t=>(<div key={t.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:6,padding:"4px 0",borderBottom:"1px solid var(--ln)",fontSize:13}}><span style={{flex:1,minWidth:0}}><span style={{color:"var(--ac)"}}>{t.tech}</span> · {t.date} · {t.hours}h @ ${t.rate}{t.notes?<span style={{color:"var(--mt)"}}> · {t.notes}</span>:""}</span><span style={{fontWeight:600,width:62,textAlign:"right"}}>{$$((+t.hours||0)*(+t.rate||0))}</span><button className="rc-bs" onClick={()=>d({type:"MODAL",v:"edit-time",d:t})} style={{fontSize:14,padding:"2px 6px"}}>✎</button><button className="rc-bs rc-bsr" onClick={()=>d({type:"DELETE",list:"timeEntries",id:t.id})} style={{fontSize:14,padding:"2px 6px"}}>×</button></div>))}</div>:<div style={{fontSize:13,color:"var(--mt)",fontStyle:"italic",margin:"2px 0 8px"}}>No time logged yet.</div>}<div className="rc-fa">{C}{te.length>0&&<button className="rc-bs" onClick={()=>d({type:"MODAL",v:"add-inv",d:{custId:j.custId,prefillItems:te.map(t=>({d:"Labor — "+t.tech+(t.date?" "+t.date:""),q:+t.hours||0,r:+t.rate||0}))}})}>🧾 Bill Labor</button>}{j.status!=="complete"&&<button className="rc-ba" onClick={()=>{d({type:"UPDATE",list:"jobs",id:j.id,d:{status:nx[j.status]}});d({type:"CLOSE"});}}>→ {nx[j.status]==="in-progress"?"In Progress":"Complete"}</button>}</div></div>);}
  if(s.modal==="cust-detail"){const c=s.md;return W(<div><div className="rc-mt">Customer</div><div style={{fontFamily:"var(--fd)",fontWeight:700,fontSize:21}}>{c.name}</div><div style={{fontSize:14,color:"var(--ac)"}}>{c.type} {c.province?`· ${c.province}`:""}</div><div style={{fontSize:14,color:"var(--tx2)",margin:"6px 0 12px"}}>{c.phone} · {c.email}</div><div className="rc-3c" style={{marginBottom:12}}><div><div className="rc-ml">Spent</div><div className="rc-mv">{$K(c.spent||0)}</div></div><div><div className="rc-ml">Visits</div><div className="rc-mv">{c.visits||0}</div></div><div><div className="rc-ml">Last</div><div className="rc-mv" style={{fontSize:14.5}}>{c.last||"—"}</div></div></div>{(c.vehicles||[]).length>0&&<div className="rc-fg"><div className="rc-fl">Vehicles</div><div style={{fontSize:14,color:"var(--tx2)"}}>{c.vehicles.join(" · ")}</div></div>}{c.notes&&<div className="rc-fg"><div className="rc-fl">Notes</div><div style={{fontSize:14,color:"var(--tx2)"}}>{c.notes}</div></div>}{(s.jobs||[]).filter(j=>j.custId===c.id).length>0&&<><div className="rc-fl" style={{marginTop:10}}>Jobs</div>{(s.jobs||[]).filter(j=>j.custId===c.id).map(j=>(<div key={j.id} style={{display:"flex",justifyContent:"space-between",padding:"4px 0",borderBottom:"1px solid var(--ln)",fontSize:14}}><span>{j.service}</span><Badge s={j.status}/></div>))}</>}{(s.invoices||[]).filter(inv=>inv.custId===c.id).length>0&&<><div className="rc-fl" style={{marginTop:10}}>Invoices</div>{(s.invoices||[]).filter(inv=>inv.custId===c.id).map(inv=>(<div key={inv.id} style={{display:"flex",justifyContent:"space-between",padding:"4px 0",borderBottom:"1px solid var(--ln)",fontSize:14,gap:6}}><span style={{color:"var(--ac)"}}>{inv.invNum}</span><span>{$$(invTot(inv))}</span><Badge s={inv.status}/></div>))}</>}<div className="rc-fa">{C}<button className="rc-bs" onClick={()=>d({type:"MODAL",v:"edit-cust",d:c})}>✎ Edit</button></div></div>);}
  if(s.modal==="inv-detail"){const inv=s.md;const sb=(inv.items||[]).reduce((a,i)=>a+i.q*i.r,0);return W(<div><div className="rc-mt">{inv.invNum||inv.id}</div><div style={{display:"flex",justifyContent:"space-between",marginBottom:12}}><div><div className="rc-fl">Customer</div><div style={{fontFamily:"var(--fd)",fontWeight:600}}>{cn(s.customers,inv.custId)}</div></div><div style={{textAlign:"right"}}><div className="rc-fl">Status</div><Badge s={inv.status}/></div></div><div className="rc-fl">Line Items</div><div style={{background:"var(--sf2)",borderRadius:6,padding:10,marginBottom:12}}>{(inv.items||[]).map((it,i)=>(<div key={i} style={{display:"flex",justifyContent:"space-between",padding:"5px 0",borderBottom:i<inv.items.length-1?"1px solid var(--ln)":"none",fontSize:14}}><span style={{flex:1}}>{it.d}</span><span style={{width:40,textAlign:"center",color:"var(--mt)"}}>×{it.q}</span><span style={{width:80,textAlign:"right",fontWeight:600}}>{$$(it.q*it.r)}</span></div>))}</div>{[["Subtotal",sb],["GST",sb*.05],["Total",sb*1.05]].map(([l,v],i)=>(<div key={i} style={{display:"flex",justifyContent:"space-between",padding:"2px 0",fontSize:i===2?14:12,fontWeight:i===2?700:400,color:i===2?"var(--ac)":"var(--tx2)"}}><span>{l}</span><span>{$$(v)}</span></div>))}<div className="rc-fa">{C}{inv.status==="pending"&&<button className="rc-ba" onClick={()=>{d({type:"UPDATE",list:"invoices",id:inv.id,d:{status:"paid"}});d({type:"CLOSE"});}}>Mark Paid</button>}</div></div>);}
  if(s.modal==="quote-detail"){const q=s.md;const sb=qTot(q);return W(<div><div className="rc-mt">{q.quoteNum||"Quote"}</div><div style={{display:"flex",justifyContent:"space-between",marginBottom:12}}><div><div className="rc-fl">Customer</div><div style={{fontFamily:"var(--fd)",fontWeight:600}}>{cn(s.customers,q.custId)}</div></div><Badge s={q.status}/></div>{q.description&&<div style={{fontSize:14,color:"var(--tx2)",marginBottom:10}}>{q.description}</div>}<div className="rc-fl">Items</div><div style={{background:"var(--sf2)",borderRadius:6,padding:10,marginBottom:12}}>{(q.items||[]).map((it,i)=>(<div key={i} style={{display:"flex",justifyContent:"space-between",padding:"5px 0",borderBottom:i<q.items.length-1?"1px solid var(--ln)":"none",fontSize:14}}><span style={{flex:1}}>{it.d}</span><span style={{width:80,textAlign:"right",fontWeight:600}}>{$$(it.q*it.r)}</span></div>))}</div>{[["Sub",sb],["GST",sb*.05],["Total",sb*1.05]].map(([l,v],i)=>(<div key={i} style={{display:"flex",justifyContent:"space-between",padding:"2px 0",fontSize:i===2?14:12,fontWeight:i===2?700:400,color:i===2?"var(--ac)":"var(--tx2)"}}><span>{l}</span><span>{$$(v)}</span></div>))}<div className="rc-fa">{C}{q.status==="approved"&&<button className="rc-ba" onClick={()=>{d({type:"ADD",list:"invoices",d:{invNum:"INV-"+Date.now().toString().slice(-6),custId:q.custId,date:today(),due:"Net 30",items:q.items,status:"pending"}});d({type:"CLOSE"});}}>→ Invoice</button>}</div></div>);}
  if(s.modal==="appt-detail"){const a=s.md;return W(<div><div className="rc-mt">Appointment</div>{[["Customer",cn(s.customers,a.custId)],["Service",a.service],["Date",a.date],["Time",a.time],["Tech",a.tech]].map(([l,v],i)=>(<div key={i} className="rc-fg"><div className="rc-fl">{l}</div><div style={{fontSize:14.5}}>{v}</div></div>))}<div className="rc-fg"><div className="rc-fl">Status</div><Badge s={a.status}/></div><div className="rc-fa">{C}{a.status==="pending"&&<button className="rc-ba" onClick={()=>{d({type:"UPDATE",list:"schedule",id:a.id,d:{status:"confirmed"}});d({type:"CLOSE"});}}>Confirm</button>}</div></div>);}
  ...
  if(s.modal==="add-job")return W(<div><div className="rc-mt">New Job</div>{CS()}{ES()}<p style={{fontSize:11,color:"var(--mt)",margin:"-4px 0 8px"}}>Link an engine and logged labor feeds its cost basis. Reman jobs need no customer.</p>{[["vehicle","Vehicle"],["service","Service"],["type","Type"]].map(([k,l])=>F(k,l))}{TS()}{[["due","Due Date"],["priority","Priority (low/medium/high)"],["notes","Notes"]].map(([k,l])=>F(k,l))}<div className="rc-fa">{X}<button className="rc-ba" onClick={()=>(f.custId||f.engineId)&&f.service&&d({type:"ADD",list:"jobs",d:{...f,custId:+f.custId||0,engineId:f.engineId?+f.engineId:null,priority:f.priority||"medium",tech:f.tech||"Unassigned",status:"queued"}})}>Save</button></div></div>);
  ...
  if(s.modal==="part-detail"){const i=engById(s,s.md&&s.md.id)||s.md;const eng=isEngine(i);const cb=costBasis(i);const tm=trueMargin(i);const mp=marginPct(i);
    ...
    return W(<div><div className="rc-mt" style={{marginBottom:4}}>Engine Unit Record</div><div style={{fontSize:12,color:"var(--mt)",letterSpacing:1,marginBottom:12}}>{i.sku}</div>
      ...
      <div className="rc-card" style={{marginBottom:12,padding:12}}><div className="rc-3c"><div><div className="rc-ml">Total in</div><div className="rc-mv">{$$(cb)}</div></div><div><div className="rc-ml">List price</div><div className="rc-mv" style={{color:"var(--ac)"}}>{i.price>0?$$(i.price):"—"}</div></div><div><div className="rc-ml">True margin</div><div className="rc-mv" style={{color:tm>0?"var(--g)":"var(--r)"}}>{i.price>0?$$(tm)+" · "+mp.toFixed(0)+"%":"—"}</div></div></div><div style={{fontSize:11,color:"var(--mt)",marginTop:8}}>Full breakdown, parts and labor live on the <span style={{color:"var(--act)",cursor:"pointer"}} onClick={()=>setPtab("costs")}>Costs tab →</span></div></div>
      </>)}
      {ptab==="costs"&&(<>
      ...
      <div className="rc-card" style={{marginBottom:12,padding:12}}>
        {(()=>{const ejs=(s.jobs||[]).filter(j=>j.engineId===i.id);
          return(<>
            ...
            <button className="rc-bs" style={{marginTop:4}} onClick={()=>{const nj={id:Date.now(),engineId:i.id,custId:0,vehicle:i.sku||"",service:"Reman — "+(i.name||i.sku||"engine"),type:"Reman",tech:"Unassigned",due:"",priority:"medium",status:"in-progress",notes:""};d({type:"ADD",list:"jobs",d:nj,label:"Reman job opened"});d({type:"MODAL",v:"job-detail",d:nj});}}>🔧 + Reman Job</button>
          </>);})()}
      </div>
      </>)}
      {ptab==="diagnosis"&&(<>
      ...
      <div className="rc-card" style={{marginBottom:12,padding:12}}>
        {(()=>{const dxs=dxFor(s,i.id);const kn=issuesFor(s,i);return(<>
          {dxs.length===0?(<div style={{fontSize:13,color:"var(--mt)",marginBottom:4}}>Nothing logged yet. Every symptom, finding and fix on this unit lives here — and feeds the shop's common-issues database.</div>):dxs.map(x=>(<div key={x.id} onClick={()=>d({type:"MODAL",v:"dx-detail",d:x})} style={{display:"flex",gap:8,alignItems:"center",padding:"5px 0",borderBottom:"1px solid var(--ln2)",fontSize:13,cursor:"pointer"}}><span style={{color:"var(--mt)",fontSize:11,width:66,flexShrink:0}}>{x.date||""}</span><span style={{flex:1,minWidth:0}}><span style={{color:"var(--tx)"}}>{(x.symptoms||[]).join(", ")||"—"}</span>{x.codes&&<span style={{color:"var(--tx2)"}}> · {x.codes}</span>}{x.findings&&<div style={{color:"var(--tx2)",fontSize:12,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{x.findings}</div>}</span>{x.tech&&<span style={{color:"var(--ac)",fontSize:12}}>{x.tech}</span>}<Badge s={x.outcome||"open"}/></div>))}
        ...
        {bomSecs(bm).map(sec=>{const ls=(bm.lines||[]).filter(l=>l.sec===sec&&vis(l));if(!ls.length)return null;const isOrd=ls.every(l=>lineKind(l)==="order");return(<div key={sec} className="rc-card" style={{marginBottom:9,padding:"9px 11px"}}>
          ...
          {ls.map(l=>{const r=sheetRow(sh,l.id);const v=rowD(sh,l);const ord=lineKind(l)==="order";const off=ord&&v==="skip";return(<div key={l.id} style={{display:"flex",gap:5,alignItems:"center",padding:"4px 0",borderBottom:"1px solid var(--ln2)",flexWrap:"wrap",opacity:off?.5:1}}>
            ...
            {ord?(<button className="rc-fb" onClick={()=>setRow(l.id,{d:off?"":"skip"})} style={{fontSize:10,padding:"3px 7px",letterSpacing:.5}}>{off?"↺ put back":"✕ not on this job"}</button>):(<>
              {!v&&<span style={{fontSize:10,letterSpacing:.5,color:st.signed?"var(--w)":"var(--mt)",marginRight:2}}>{st.signed?"→ REPLACE":"blank"}</span>}
              {DISPO.map(([k,lab,col])=>(<button key={k} className="rc-fb" onClick={()=>setRow(l.id,{d:v===k?"":k})} style={{fontSize:10,padding:"3px 7px",letterSpacing:.5,...(v===k?{borderColor:col,color:col,background:tint(col,14)}:{})}}>{lab}</button>))}</>)}
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9,marginBottom:6}}>
        {lk.invoice?Lrec("🧾","var(--b)","Invoice",lk.invoice.invNum,$$(invTot(lk.invoice))+" · "+lk.invoice.status,BC[lk.invoice.status]):Lrec("🧾","var(--ln)","Invoice","Not linked","Add via Invoicing")}
        {lk.core?Lrec("🔄","var(--w)","Core",$$(lk.core.deposit||0)+" dep",engStatusLabel(lk.core.status)+(lk.core.dueDate?" · due "+lk.core.dueDate:""),BC[lk.core.status]):Lrec("🔄","var(--ln)","Core","Not linked","Add via Operations")}
        {lk.warranty?Lrec("🛡","var(--g)","Warranty",lk.warranty.warrantyPeriod||"—",lk.warranty.status+(lk.warranty.expiryDate?" · exp "+lk.warranty.expiryDate:""),BC[lk.warranty.status]):Lrec("🛡","var(--ln)","Warranty","Not linked","Add via Operations")}
        {lk.shipment?Lrec("🚚","var(--b)","Shipment",lk.shipment.carrier,(lk.shipment.deliveryConfirmed?"Delivered":"In transit")+(lk.shipment.destination?" · "+lk.shipment.destination:""),lk.shipment.deliveryConfirmed?"var(--g)":"var(--b)"):Lrec("🚚","var(--ln)","Shipment","Not linked","Add via Operations")}
      </div>
      </>)}
      ...
      <div className="rc-fa">{C}{engStatus(i)==="available"&&<button className="rc-ba" onClick={()=>{d({type:"MODAL",v:"add-inv",d:{engineId:i.id,engineName:i.name,prefillItems:[{d:i.name+" (ESN "+(i.serial||i.esn||"")+")",q:1,r:i.price||0}]}});}}>Sell Engine →</button>}<button className="rc-bs" onClick={()=>d({type:"MODAL",v:"export-engine",d:i})}>🌐 Export</button><button className="rc-bs" onClick={()=>printEngine(s,i)}>🖨 Print</button><button className="rc-bs" onClick={()=>d({type:"MODAL",v:"edit-part",d:i})}>✎ Edit</button></div></div>);}
  ...
  if(s.modal==="add-campaign")return FM("Add Campaign",[["name","Name"],["platform","Platform"],["type","Type"],["status","Status"],["reach","Reach","number"],["leads","Leads","number"],["spent","Spent ($)","number"],["budget","Budget ($)","number"],["notes","Notes"]],()=>f.name&&d({type:"ADD",list:"campaigns",d:{...f,reach:+f.reach||0,leads:+f.leads||0,spent:+f.spent||0,budget:+f.budget||0,status:f.status||"draft"}}));
  if(s.modal==="add-comm")return W(<div><div className="rc-mt">Log Communication</div>{CS()}<div className="rc-fg"><label className="rc-fl">Type</label><select className="rc-fi" value={f.type||"call"} onChange={e=>set("type",e.target.value)} style={{appearance:"none"}}><option value="call">Call</option><option value="email">Email</option><option value="text">Text</option><option value="note">Note</option></select></div>{[["date","Date"],["summary","Summary"],["followUp","Follow-Up Date"]].map(([k,l])=>F(k,l))}<div className="rc-fa">{X}<button className="rc-ba" onClick={()=>f.summary&&d({type:"ADD",list:"commsLog",d:{...f,custId:+f.custId||0,date:f.date||isoToday()}})}>Save</button></div></div>);
  if(s.modal==="add-core")return W(<div><div className="rc-mt">Add Core Return</div>{CS()}{ES(eng=>set("engineName",eng.name))}{[["engineName","Engine Name"],["deposit","Core Deposit ($)","number"],["dueDate","Due Date"],["notes","Notes"]].map(([k,l,t])=>F(k,l,t))}<div className="rc-fa">{X}<button className="rc-ba" onClick={()=>f.engineName&&d({type:"ADD",list:"cores",d:{...f,custId:+f.custId||0,engineId:+f.engineId||0,deposit:+f.deposit||0,status:"pending"}})}>Save</button></div></div>);
  if(s.modal==="add-ship")return W(<div><div className="rc-mt">Add Shipment</div>{CS()}{ES()}{[["carrier","Carrier"],["tracking","Tracking #"],["freightCost","Freight Cost ($)","number"],["shipDate","Ship Date"],["estDelivery","Est. Delivery"],["origin","Origin"],["destination","Destination"],["notes","Notes"]].map(([k,l,t])=>F(k,l,t))}<div className="rc-fa">{X}<button className="rc-ba" onClick={()=>f.carrier&&d({type:"ADD",list:"shipments",d:{...f,custId:+f.custId||0,engineId:+f.engineId||0,freightCost:+f.freightCost||0,deliveryConfirmed:false}})}>Save</button></div></div>);
  if(s.modal==="add-po")return W(<div><div className="rc-mt">New Purchase Order</div>{[["vendor","Vendor"],["orderDate","Order Date"],["eta","ETA"],["notes","Notes"]].map(([k,l])=>F(k,l))}<div className="rc-fl">Items</div>{LI()}<div style={{fontSize:14,color:"var(--tx2)",textAlign:"right",marginBottom:10}}>Total: {$$(sub)}</div><div className="rc-fa">{X}<button className="rc-ba" onClick={()=>f.vendor&&d({type:"ADD",list:"purchaseOrders",d:{vendor:f.vendor,orderDate:f.orderDate||today(),eta:f.eta,notes:f.notes,items:lines.filter(l=>l.d),status:"ordered"}})}>Save</button></div></div>);
  if(s.modal==="add-warranty")return W(<div><div className="rc-mt">Add Warranty</div>{CS()}{ES(eng=>set("engineName",eng.name))}{[["engineName","Engine Name"],["warrantyPeriod","Warranty Period (e.g. 2yr/unlimited km)"],["startDate","Start Date"],["expiryDate","Expiry Date"],["claimNotes","Notes"]].map(([k,l])=>F(k,l))}<div className="rc-fa">{X}<button className="rc-ba" onClick={()=>f.engineName&&d({type:"ADD",list:"warranties",d:{...f,custId:+f.custId||0,engineId:+f.engineId||0,status:"active"}})}>Save</button></div></div>);

  // Quote & Invoice with line items
  if(s.modal==="add-quote")return W(<div><div className="rc-mt">New Quote</div>{CS()}{F("quoteNum","Quote #")}{F("description","Description")}<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>{F("date","Date")}{F("validUntil","Valid Until")}</div><div className="rc-fl">Items</div>{LI()}{F("notes","Notes")}<div style={{fontSize:14,color:"var(--tx2)",textAlign:"right"}}>Sub: {$$(sub)} · GST: {$$(sub*.05)} · <strong style={{color:"var(--ac)"}}>Total: {$$(sub*1.05)}</strong></div><div className="rc-fa">{X}<button className="rc-ba" onClick={()=>f.description&&lines.some(l=>l.d)&&d({type:"ADD",list:"quotes",d:{quoteNum:f.quoteNum||"QT-"+Date.now().toString().slice(-6),custId:+f.custId||0,description:f.description,date:f.date||isoToday(),validUntil:f.validUntil||"30 days",items:lines.filter(l=>l.d),notes:f.notes||"",status:"draft"}})}>Save</button></div></div>);
  if(s.modal==="add-inv")return W(<div><div className="rc-mt">New Invoice</div>{CS()}{ES()}{F("invNum","Invoice #")}<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>{F("date","Date")}{F("due","Terms")}</div><div className="rc-fl">Items</div>{LI()}<div style={{fontSize:14,color:"var(--tx2)",textAlign:"right"}}>Sub: {$$(sub)} · GST: {$$(sub*.05)} · <strong style={{color:"var(--ac)"}}>Total: {$$(sub*1.05)}</strong></div><div className="rc-fa">{X}<button className="rc-ba" onClick={()=>{if(!(f.custId&&lines.some(l=>l.d)))return;const eid=+f.engineId||0;d({type:"ADD",list:"invoices",d:{invNum:f.invNum||"INV-"+Date.now().toString().slice(-6),custId:+f.custId,engineId:eid,date:f.date||isoToday(),due:f.due||"Net 30",items:lines.filter(l=>l.d),status:"pending"}});if(eid)d({type:"UPDATE",list:"inventory",id:eid,d:{status:"sold"}});d({type:"CLOSE"});}}>Create</button></div></div>);
  ...
  if(s.modal==="edit-job")return W(<div><div className="rc-mt">Edit Job</div>{CS()}{ES()}{[["vehicle","Vehicle"],["service","Service"],["type","Type"]].map(([k,l])=>F(k,l))}{TS()}{[["due","Due"],["priority","Priority"],["notes","Notes"]].map(([k,l])=>F(k,l))}<div className="rc-fg"><label className="rc-fl">Status</label><select className="rc-fi" value={f.status||"queued"} onChange={e=>set("status",e.target.value)} style={{appearance:"none"}}><option value="queued">Queued</option><option value="in-progress">In Progress</option><option value="complete">Complete</option></select></div><div className="rc-fa">{X}<button className="rc-ba" onClick={()=>{d({type:"UPDATE",list:"jobs",id:s.md.id,d:{...f,custId:+f.custId||0,engineId:f.engineId?+f.engineId:null}});d({type:"CLOSE"});}}>Save</button></div></div>);
  ...
  if(s.modal==="edit-inv")return W(<div><div className="rc-mt">Edit Invoice</div>{CS()}{F("invNum","Invoice #")}<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>{F("date","Date")}{F("due","Terms")}</div><div className="rc-fg"><label className="rc-fl">Status</label><select className="rc-fi" value={f.status||"pending"} onChange={e=>set("status",e.target.value)} style={{appearance:"none"}}><option value="pending">Pending</option><option value="paid">Paid</option><option value="overdue">Overdue</option></select></div><div className="rc-fa">{X}<button className="rc-ba" onClick={()=>{d({type:"UPDATE",list:"invoices",id:s.md.id,d:{invNum:f.invNum,custId:+f.custId||s.md.custId,date:f.date,due:f.due,status:f.status}});d({type:"CLOSE"});}}>Save</button></div></div>);
  ...
  if(s.modal==="add-content")return FM("Add Calendar Entry",[["day","Day (Mon/Tue/Wed/Thu/Fri/Sat/Sun)"],["type","Content Type"],["platform","Platform(s)"],["notes","Notes"]],()=>f.day&&f.type&&d({type:"ADD",list:"contentCalendar",d:{day:f.day,type:f.type,platform:f.platform||"",notes:f.notes||""}}));
  ...
  if(s.modal==="dx-detail"){const x=(s.diagnoses||[]).find(y=>y.id===s.md.id)||s.md;const eng=engById(s,x.engineId);const is=x.issueId?(s.issues||[]).find(y=>y.id===+x.issueId):null;const job=x.jobId?(s.jobs||[]).find(j=>j.id===+x.jobId):null;const pv=(x.parts||[]).reduce((a,p)=>a+(+p.v||0),0);
    return W(<div><div className="rc-mt" style={{marginBottom:4}}>Diagnosis</div><div style={{fontSize:12,color:"var(--mt)",letterSpacing:1,marginBottom:10}}>{x.date}{x.tech?" · "+x.tech:""}</div>
      ...
      <div className="rc-3c" style={{marginBottom:12}}><div><div className="rc-ml">Hours</div><div className="rc-mv">{+x.hours||0}h{+x.rate>0&&+x.hours>0?<span style={{fontSize:13,color:"var(--tx2)"}}> · {$$((+x.hours||0)*(+x.rate||0))}</span>:null}</div></div><div><div className="rc-ml">Parts</div><div className="rc-mv">{$$(pv)}</div></div><div><div className="rc-ml">Work order</div><div className="rc-mv" style={{fontSize:14}}>{job?<span style={{cursor:"pointer",color:"var(--ac)"}} onClick={()=>d({type:"MODAL",v:"job-detail",d:job})}>{job.service}</span>:"—"}</div></div></div>
      ...
      {is?<div className="rc-card" style={{padding:10,marginBottom:10,cursor:"pointer"}} onClick={()=>d({type:"MODAL",v:"issue-detail",d:is})}><div className="rc-ml">Linked common issue</div><div style={{fontSize:14,color:"var(--ac)"}}>{is.title}</div></div>:null}
  ...
  if(s.modal==="issue-detail"){const is=(s.issues||[]).find(y=>y.id===s.md.id)||s.md;const seen=(s.diagnoses||[]).filter(x=>+x.issueId===is.id).sort((a,b)=>(b.date||"").localeCompare(a.date||""));const fits=(s.inventory||[]).filter(isEngine).filter(e=>engStatus(e)!=="sold"&&issueFits(is,e));
    return W(<div><div className="rc-mt" style={{marginBottom:4}}>Common Issue</div><div style={{display:"flex",gap:6,alignItems:"center",marginBottom:8}}><Badge s={is.severity||"medium"}/><Badge s={is.source||"shop"}/></div>
      ...
      <div style={{fontSize:13,color:"var(--tx2)",marginBottom:8}}>Applies to: <span style={{color:"var(--ac)"}}>{(is.models||[]).length?is.models.map(famLabel).join(", "):"every engine"}</span></div>
...
.rc-root[data-theme="night"]{--bg:#121314;--sf:#1a1c1f;--sf2:#212428;--in:#15171a;--ln:#2d3136;--ln2:#25282c;--tx:#ecebe7;--tx2:#c8c4bc;--mt:#9a958c;--ft:#6e6961;--ac:#e2682a;--act:#ff9058;--acs:rgba(226,104,42,.16);--ach:#ef7836;--g:#4cc38a;--gs:rgba(76,195,138,.14);--w:#e2b24c;--ws:rgba(226,178,76,.14);--r:#f06b58;--rs:rgba(240,107,88,.15);--b:#6ea9ea;--bs:rgba(110,169,234,.15);--p:#b28ceb;--ps:rgba(178,140,235,.15);--sh1:0 1px 2px rgba(0,0,0,.35);--sh2:0 16px 40px -18px rgba(0,0,0,.85);--ov:rgba(0,0,0,.62);--grad:linear-gradient(135deg,#f07a3a,#e2682a);color-scheme:dark;}
...
.rc-side{position:sticky;top:0;height:100vh;overflow-y:auto;background:var(--sf);border-right:1px solid var(--ln);display:flex;flex-direction:column;padding:18px 14px 14px;z-index:60;}
...
.rc-navlist{display:flex;flex-direction:column;gap:1px;}
.rc-nsec{font-size:10.5px;font-weight:700;letter-spacing:1.4px;text-transform:uppercase;color:var(--ft);padding:16px 10px 5px;}
...
.rc-sfoot{margin-top:auto;padding-top:14px;border-top:1px solid var(--ln);display:flex;flex-direction:column;gap:10px;}
...
.rc-ba{display:inline-flex;align-items:center;justify-content:center;gap:6px;padding:9px 16px;border:1px solid transparent;border-radius:9px;background:var(--ac);color:#fff;font-size:14px;font-weight:600;cursor:pointer;transition:background .12s;box-shadow:0 1px 2px rgba(150,60,10,.22);}
.rc-ba:hover{background:var(--ach);}
...
.rc-rboard{display:grid;grid-template-columns:repeat(5,minmax(180px,1fr));gap:12px;margin-bottom:18px;overflow-x:auto;padding-bottom:6px;}
...
.rc-bomtick{font-size:10px;padding:3px 6px;}
...
export default function App(){
  ...
  const TABL={overview:"Overview",inventory:"Engines",parts:"Parts",boms:"BOM",social:"Marketing",issues:"Issues",customers:"Customers & Jobs",operations:"Operations",schedule:"Schedule",quotes:"Quotes",invoices:"Invoicing",employees:"Team",reports:"Reports"};
  const NAV=[{t:"overview",i:"home"},{h:"Shop"},{t:"inventory",i:"eng"},{t:"boms",i:"clip"},{t:"parts",i:"box"},{t:"issues",i:"wrench"},{h:"Sales"},{t:"social",i:"mega"},{t:"quotes",i:"doc"},{t:"invoices",i:"cash"},{h:"Operations"},{t:"customers",i:"users"},{t:"operations",i:"truck"},{t:"schedule",i:"cal"},{h:"Business"},{t:"employees",i:"team"},{t:"reports",i:"chart"}];
