...
src/
  ...
  bomSeed.js                     ISX15 CM2350 parts & disposition worksheet (BOM_SEED) + suppliers (VENDOR_SEED)
  lib/
...
- **Bill of materials / teardown worksheet.** `boms` are per-family templates (the shop's paper *Parts & Disposition Worksheet*): `{id, label, family, model, rev, match:[], note, rule, watch, lines:[{id, sec, qty, part, tier, note, mach}]}`. `tier` encodes the two always-replace lists on the back of that form — `1` = ordered without inspection, `2` = reuse only with a measurement written down, `0` = inspect and decide. `bomSheets` is one filled-in worksheet per engine: `{id, engineId, bomId, engName, date, tech, wo, coreSource, notes, rows:{[lineId]:{d, cost, pn, url, meas, logged}}}`, where `d` is the one tick per line — `miss` (not with the core) / `reuse` / `repl` / `mach`. Starting a sheet pre-ticks every tier-1 line REPL (`newRows`). `bomFits`/`bomsFor` match a template to an engine by `match` tokens against the normalized name, exactly like `issueFits`. Helpers: `bomStats` (counts + progress), `bomPick` (lines by tick), `bomSecs`, `bomCost`. The passport's **BOM** tab is the tick sheet; **Shopping list** (`bom-shop`) is everything ticked REPL with a cost box and buy links, and its *Log to cost basis* appends straight to the engine's `partsLog` (marking `logged` so nothing double-counts); **Machine shop** (`bom-mach`) is everything ticked MACH, deduped and printable. Seeded from `bomSeed.js` (ISX15 CM2350, 87 lines, 11 sections). The seed's match token is **`X15`**, which catches both the older ISX15 and the 2017+ X15 (same 15L family, CM2350-era ECM) but deliberately not the CM870/871 ISX — different fuel system, clone the worksheet for those. Manage templates and suppliers in Inventory → BOM (`Boms` view): each template shows exactly which engines on the lot it matches (so a token that matches nothing is visible, not silent), a **Put this worksheet on an engine** picker starts a sheet and jumps to that engine's passport, and **What The Shop Needs** rolls every ticked line from every worksheet into one filterable list (needs replacing / missing from the core / to the machine shop / reusing). The passport never dead-ends: when no template matches, it offers the other worksheets to use anyway plus a Build one shortcut. Clone a template to start another family. Put the wrong worksheet on an engine? **✕ Remove worksheet** sits in the BOM tab header (and as ✕ on each row of *Worksheets In Progress*); it's a plain `DELETE`, so the toast undoes it with every tick intact. **Sheet info → Wrong worksheet? Swap it** moves the sheet to a different template instead, carrying ticks across by normalized part name and pre-ticking tier 1 for whatever is new — use it when the parts overlap, Remove when they don't.
...
6. Printable modals (shopping list, machine shop list) pass `"rc-pmod"` as the second argument to `W`; the print stylesheet re-shows that overlay only.
